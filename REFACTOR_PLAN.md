...
### 第12轮：协调器领域拆分（已完成 2026-08-03）

- 新增 `parsers/`：以 `StatePatch` 为统一接口，按灯光/窗帘、传感器、
  空调/新风/晾衣架、门锁拆分纯状态解析器，并用 `DeviceCategory` 注册表路由。
- 人体传感器与紧急按钮的值解析保持纯函数；延时复位任务仍由协调器负责，
  明确区分协议解析与 Home Assistant 生命周期副作用。
- 新增 `lock_manager.py`：负责门锁事件归一化、去重、最近开锁关联、开门归属和
  cmd=82 消息分类；协调器只附加媒体 URL 并发布 HA 事件。
- 新增 `control_router.py`：电源、亮度、色温的设备分类分流改为纯路由决策，
  协调器只执行选定方法、等待响应并应用乐观状态。
- type 107 门锁状态推送接入门锁解析器；cmd=352 瞬态事件复位使用本地匹配后的
  device_id，兼容云端 ID 与本地 `w-` ID 不一致的设备。
